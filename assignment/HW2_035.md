...
```{R}
mean(cats$Hwt[cats$Sex == 'F']) #Mean of Hwt Female cat = 9.202128

mean(cats$Hwt[cats$Sex == 'M']) #Mean of Hwt Male cat = 11.32268
```
...
```{R}
max(cats$Bwt) #Max of Bwt cat = 20.5

min(cats$Bwt) #Min of Bwt cat = 6.3
```

4.) หา Body Weight ที่ซ้ำกันมากที่สุดของแมวทั้งหมด และมีจำนวนเท่าไหร่
```{R}
Mode(cats$Bwt) #Mode of Bwt cat = 2.2 and sum of mode = 14
```
